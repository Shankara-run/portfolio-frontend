 ...
 // const [error, setError] = useState("");
  ...
  return (
    <div className="h-full flex flex-col space-y-3">
      {profile.map((p, index) => (
        <div key={index} className="text-center space-y-3">
          ...
          <div className="relative mx-auto w-fit">
            <div className={`absolute inset-0 rounded-full blur-xl opacity-30 ${showHidden ? 'bg-red-400' : 'bg-blue-400'}`}></div>
            <img
              src={p.image}
              alt="Profile"
          ...
          <div className="flex justify-center gap-3 mt-3">
            ...
            >
              {showHidden ? <FaBehance className="text-xs" /> : <FaGithub className="text-xs" />}
            </a>
          </div>
        </div>
      ))}
              ...
              className={`relative flex items-center space-x-2 px-2 py-1.5 rounded-md backdrop-blur-sm
                ...
                ${isActive
                  ? `${showHidden
                    ...
                    } ring-1 ${showHidden ? 'ring-red-400/30' : 'ring-blue-400/30'}`
                  : `bg-white/10 border-white/20 hover:bg-white/20 ${showHidden ? 'hover:border-red-300/50' : 'hover:border-blue-300/50'}`
                } overflow-hidden`}
            >
              {/* Glow effect for active item */}
              {isActive && (
                <div className={`absolute inset-0 ${showHidden ? 'bg-red-400' : 'bg-blue-400'}
              ...
              <BiRightArrow
                className={`relative z-10 transition-all duration-300 ${
                  isActive
                    ? `${showHidden ? 'text-red-200' : 'text-blue-200'} transform rotate-90 scale-110`
                    : `${showHidden ? 'text-red-300' : 'text-blue-300'}`
                }`}
              />
              <span className={`relative z-10 text-sm font-medium transition-all duration-300 ${
                isActive ? 'text-white font-bold drop-shadow-sm' : 'text-white/90'
              }`}>
                {item.name}
              </span>
              {isActive && (
                <div className={`relative z-10 ml-auto w-2 h-2 rounded-full ${showHidden ? 'bg-red-300' : 'bg-blue-300'}
                  animate-pulse shadow-lg ${showHidden ? 'shadow-red-300/50' : 'shadow-blue-300/50'}`}>
                </div>
              )}
            </div>
          );
      ...
      {/* Profile Toggle Button - At the bottom */}
      <div className="mt-4 pt-3 border-t border-white/20">
        ...
        >
          ...
          <div className="flex items-center space-x-1.5">
            <span className={`text-xs ${showHidden ? 'text-red-200' : 'text-blue-200'}`}>
              {showHidden ? 'Biz' : 'Tech'}
            </span>
            <div className={`w-10 h-5 rounded-full relative transition-all duration-300
              ${showHidden ? 'bg-red-600' : 'bg-blue-600'}`}>
              <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all duration-300 transform shadow-md
